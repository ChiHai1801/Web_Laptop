<?php
include('../../layouts/header.php');
include('../../Model/TNdatbiet.php');

if (isset($_SESSION['success'])) {
    unset($_SESSION['success']);
}
$TNdatbiet = new TNdatbiet();

if (isset($_GET['id'])) {
    $id = $_GET['id'];
    if (is_numeric($id)) {
        $obj = $TNdatbiet->gettinhnangdatbietById($id);
    } else {
        header('Location:index.php');
    }
}

if (isset($_POST['tn_id'])) {
    $id = $_POST['tn_id'];
    $count = $TNdatbiet->update($_POST);
    $_SESSION['success'] = "Sua thanh cong " . ' san pham';
}
?>

<body>
    <div class="main_containers">
        <?php
        if (isset($_SESSION['success'])) {
        ?>
            <div class="alert alert-primary" role="alert">
                <?php echo $_SESSION['success'] ?>
            </div>
        <?php }?>
        <form method="post" enctype="multipart/form-data">

            <input type="hidden" name="tn_id" value="<?php echo $obj['tn_id'] ?>" required />
            <div class="form-group row">
                <label for="staticEmail" class="col-sm-2 col-form-label">Tính năng đặt biệt:</label>
                <div class="col-sm-10">
                    <input type="text" name="TNDBName" value="<?php echo $obj['tn_name'] ?>" >
                </div>
            </div>

            <input type="submit" class="btn btn-primary"></input>
        </form>
    </div>
</body>
<?php
include('../../layouts/footer.php');
?>